...
def read_version():
    package = ET.parse("autoware_lanelet2_extension/package.xml")
...
class Lanelet2Conan(ConanFile):
    ...
    exports_sources = "*"
    exports = "autoware_lanelet2_extension/package.xml"
    ...
    def build(self):
        ...
        if self.options.build_wheel:
            with (
                Path(self.source_folder) / "autoware_lanelet2_extension_python" / "setup.py.template"
            ).open() as f:
    ...
    def _set_env(self, env_info):
        if not self.package_folder:
            return
        execs = ("autoware_lanelet2_extension_python")
